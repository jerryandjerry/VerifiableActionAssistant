...
from vaa.schemas import WorkspaceContext
...
def workspace_context(session: Session = Depends(get_session)) -> WorkspaceContext:
...
@router.get("/tickets")
def workspace_tickets(session: Session = Depends(get_session)) -> list[dict[str, object]]:
    return list_tickets(session)


@router.post("/reset")
def reset_workspace(request: Request) -> dict[str, str]:
    if not request.app.state.settings.workspace_reset_enabled:
        raise PolicyViolationError("Workspace reset is disabled")
