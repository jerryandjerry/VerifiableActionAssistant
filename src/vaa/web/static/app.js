...
async function loadContext() {
  context = await api("/api/workspace/context");
...
function renderAction(task) {
  ...
  if (!task.action_preview) {
    area.innerHTML = `<p class="muted">This task is read-only. No ticket write was proposed.</p>`;
    return;
  }
...
async function decide(action) {
  try {
    const task = await api(`/api/tasks/${currentTask.id}/${action}`, {
      method: "POST",
      body: JSON.stringify({ actor_id: "alice", comment: `Workflow ${action}` }),
    });
    renderTask(task);
...
$("reset-button").addEventListener("click", async () => {
  await api("/api/workspace/reset", { method: "POST", body: "{}" });
  currentTask = null;
  $("task-view").classList.add("hidden");
  $("empty-state").classList.remove("hidden");
  showToast("Workspace data reset");
});
